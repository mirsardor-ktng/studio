...
const onesRuFeminine = [ // For thousands and potentially kopecks if needed
...
/**
 * Helper function to get the correct Russian noun declension based on the number.
 * @param num The number.
 * @param one Form for 1 (e.g., 'рубль').
 * @param two Form for 2-4 (e.g., 'рубля').
 * @param five Form for 5-9, 0, 11-19 (e.g., 'рублей').
...
/**
 * Converts a non-negative number into Russian words, specifically formatted for currency (rubles and kopecks).
...
export function numberToWords(num: number): string {
    ...
    // Handle potential rounding issues leading to 100 kopecks
    if (decimalPart === 100) {
        integerPart += 1;
        decimalPart = 0;
    }
        ...
        do {
            ...
            if (chunk !== 0) {
                ...
                if (i > 0) {
                    ...
                    if (i === 1 && chunk === 1) {
                         thousandWord = 'тысяча'; // Already feminine 'одна' is generated
                    } else if (i > 0) {
    ...
    const rubleDeclension = getRussianNounDeclension(Math.floor(num), 'рубль', 'рубля', 'рублей'); // Use original integer part for declension

     // Format decimal part always with two digits
    const decimalFormatted = decimalPart.toString().padStart(2, '0');
    const kopeckDeclension = getRussianNounDeclension(decimalPart, 'копейка', 'копейки', 'копеек');

    // Combine parts
    const finalWords = `${integerWords.trim()} ${rubleDeclension} ${decimalFormatted} ${kopeckDeclension}`;
